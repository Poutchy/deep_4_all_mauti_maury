...
class DASPipelineQwen:
    ...
    def calculate_sentence_scores(self, user_prompt, teacher_text, teacher_logprobs):
        ...
        inputs = self.tokenizer(full_input_str, return_tensors="pt").to(
            self.model.device
        )
        ...
        prompt_tokens_len = len(
            self.tokenizer(prompt_str, add_special_tokens=False)["input_ids"]
        )
        ...
        for sent in sentences:
            ...
            p_teacher = (
                np.exp(np.mean(sent_openai_logprobs)) if sent_openai_logprobs else 0
            )
    ...
    def run_das(self, prompt):
        ...
        try:
            ...
            for i, s in enumerate(stats):
                ...
                print(
                    f"[{i}] T={s['p_teacher']:.2f} S={s['p_student']:.2f} | Diff={divergence:.2f} {tag}"
                )
